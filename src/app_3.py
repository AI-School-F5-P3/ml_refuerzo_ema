...
if menu == "Introducción":
    ...
    metrics_files = [f for f in os.listdir(metrics_dir) if f.endswith('.png')]  # Solo archivos PNG

    if metrics_files:
        st.markdown("### Gráficas Disponibles")
        
        # Mostrar todas las gráficas encontradas
        for graph in metrics_files:
            st.image(os.path.join(metrics_dir, graph), caption=graph)
    else:
        st.error("No se encontraron archivos PNG en el directorio '../src/metrics'.")
