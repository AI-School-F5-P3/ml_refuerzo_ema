...
def convert_classification_report_to_dict(y_true, y_pred, target_names):
    """
    Convert sklearn's classification_report to a dictionary for JSON serialization.
    ...
    Returns:
    - dict: Structured classification report
    """
...
plt.ylabel('Precisión')
plt.ylim(0, 1)
plt.savefig('../src/metrics/rn_fold_accuracies.png')
plt.show()
...
target_names = [str(i) for i in encoder.categories_[0]]
report_dict = convert_classification_report_to_dict(y_true, y_pred, target_names)
...
with open("../src/metrics/classification_report.json", "w") as f:
    json.dump(report_dict, f, indent=4)  # Corregido de classification_report_dict a report_dict
...
with open("../src/metrics/rn_metrics.json", "w") as f:
